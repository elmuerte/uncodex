...
uses
  Contnrs ,IniFiles, unit_definitions;
...
procedure SIRegister_TMemIniFile(CL: TPSPascalCompiler);
begin
  with CL.AddClass(CL.FindClass('TObject'),TMemIniFile) do
  begin
...
procedure SIRegister_TObjectList(CL: TPSPascalCompiler);
begin
  with CL.AddClass(CL.FindClass('TList'),TObjectList) do
  begin
    RegisterPublishedProperties;
...
begin
  ...
  begin
    ...
    //SetDefaultPropery('Items'); //TODO: broken?
...
procedure SIRegister_TLogEntry(CL: TPSPascalCompiler);
begin
  with CL.AddClass(CL.FindClass('TObject'),TLogEntry) do
  begin
    RegisterPublishedProperties;
...
(*----------------------------------------------------------------------------*)
{$IFDEF FPC}
function TMemIniFileCreate(const FileName : string): TMemIniFile;
begin
...
begin
  ...
  begin
    {$IFDEF FPC}
    RegisterConstructor(@TMemIniFileCreate, 'Create');
    {$ELSE}
